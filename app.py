...
st.markdown(
    ...
    .stApp {
        background: radial-gradient(circle at 50% 0%, #29200f 0, #111 30%, #050505 72%);
        color: #f7f2e7;
    }
    ...
    for _, cita in citas.iterrows():
        ...
        try:
            inicio = datetime.combine(
                fecha,
                datetime.strptime(str(cita["Hora"]).strip(), "%H:%M").time(),
            )
...
def mostrar_encabezado():
    ...
    st.markdown(
        '<p class="master-subtitle">Reserva tu momento de belleza.</p>',
        unsafe_allow_html=True,
    )
...
def main():
    ...
    try:
        ...
        st.warning(
            "La agenda está visible, pero Google Sheets no respondió. "
            "Las reservaciones se reactivarán cuando vuelva la conexión."
        )
